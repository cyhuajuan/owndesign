...
describe('WorkspaceStore', () => {
  ...
  it('searches Project Workspace text files with line previews', async () => {
    ...
    await expect(store.searchProjectWorkspace(project.id, 'Dashboard')).resolves.toEqual({
      matches: [
        {
          line: 2,
          path: 'index.html',
  ...
  it('matches Project Workspace paths with glob patterns', async () => {
    ...
    await expect(store.globProjectWorkspace(project.id, '**/*.{css,js}')).resolves.toMatchObject({
  ...
  it('reads, writes, edits, and deletes Project Workspace paths', async () => {
    ...
    await expect(store.readProjectWorkspaceFile(project.id, 'pages/home.html')).resolves.toBe(
      'Hello world',
    );
    await expect(
      store.editProjectWorkspaceFile(project.id, 'pages/home.html', 'Hello', 'Design'),
    ).resolves.toMatchObject({
    ...
    await expect(store.deleteProjectWorkspacePath(project.id, 'pages')).resolves.toEqual({
      deleted: true,
      path: 'pages',
    });
    await expect(store.readProjectWorkspaceFile(project.id, 'pages/home.html')).rejects.toThrow();
  });
  ...
  it('applies Project Workspace patch changes', async () => {
    ...
    await expect(store.readProjectWorkspaceFile(project.id, 'assets/app.css')).resolves.toBe(
      'body {}',
    );
    await expect(store.readProjectWorkspaceFile(project.id, 'remove.txt')).rejects.toThrow();
  });
  ...
  it('does not partially apply Project Workspace patch changes when validation fails', async () => {
    ...
    await expect(store.readProjectWorkspaceFile(project.id, 'created.txt')).rejects.toThrow();
    await expect(store.readProjectWorkspaceFile(project.id, 'index.html')).resolves.toBe(
      '<main>Old</main>',
    );
  });
  ...
  it('returns truncation and skipped-file metadata for large reads and greps', async () => {
    ...
    await store.writeProjectWorkspaceFile(project.id, 'huge.txt', 'Dashboard\n'.repeat(130_000));
    await writeFile(
      path.join(workspaceRoot, 'projects', project.id, 'workspace', 'binary.bin'),
      Buffer.from([0, 1, 2, 3]),
    );

    await expect(store.readProjectWorkspaceEntry(project.id, 'long.html')).resolves.toMatchObject({
      content: expect.stringContaining('<truncated>'),
      truncated: true,
      truncatedReason: 'line-length',
    });
    await expect(store.grepProjectWorkspace(project.id, 'Dashboard')).resolves.toMatchObject({
      matches: [],
  ...
  it('rejects unsafe Project Workspace paths', async () => {
    ...
    await expect(store.deleteProjectWorkspacePath(project.id, '')).rejects.toThrow(
      'must not be empty',
    );
  });
  ...
  it('rejects symlink access in the Project Workspace', async () => {
    ...
    try {
      await symlink(
        ...
        path.join(workspaceRoot, 'projects', project.id, 'workspace', 'secret-link.txt'),
      );
    } catch (error) {
    ...
    await expect(store.readProjectWorkspaceFile(project.id, 'secret-link.txt')).rejects.toThrow(
  ...
  it('reads Project Workspace files as buffers', async () => {
    ...
    await expect(store.readProjectWorkspaceFileBuffer(project.id, 'index.html')).resolves.toEqual(
      Buffer.from('<main>Buffer</main>'),
    );
  });
