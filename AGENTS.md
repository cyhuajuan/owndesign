...
- The agent edits files only through the **workspace tools** in `src/agent/tools/` (`read`, `glob`, `grep`, `edit`, `patch`, `write`, `createHtml`, `copyFile`, `delete`, `preview-refresh`), registered via `createWorkspaceToolRegistry`. These are sandboxed to the project workspace and CDN-guarded (`cdn-guard.ts` restricts external URLs to user-approved font/icon CDNs).
