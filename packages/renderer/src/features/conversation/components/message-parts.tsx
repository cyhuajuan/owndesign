...
import { isToolPart, ToolPartView } from '@/features/conversation/components/tool-part-view';
...
export function MessageParts({
  ...
  const originalUserPrompt = message.role === 'user' ? getOriginalUserPrompt(message) : undefined;
  const streamingReasoningPartIndex =
    isLastMessage && isStreaming && lastPart?.type === 'reasoning' ? message.parts.length - 1 : -1;
  const useStreamingText = isLastMessage && isStreaming && message.role === 'assistant';

  return (
    <>
      {originalUserPrompt ? <PlainTextResponse>{originalUserPrompt}</PlainTextResponse> : null}
...
function PlainTextResponse({ children }: { children: string }) {
  return <div className="size-full whitespace-pre-wrap break-words">{children}</div>;
}

function StreamingTextResponse({ children }: { children: string }) {
  return (
    <div className="size-full whitespace-pre-wrap break-words" data-streaming-text="true">
      {children}
    </div>
  );
}
