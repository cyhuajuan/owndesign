...
export function buildPageTargetProtocolPrompt() {
  return [
    ...
    "- After `createHtml` succeeds, use `read` on that file, then use `edit` or `patch` to fill in the actual page design.",
...
export function buildToolWorkflowPrompt() {
  return [
    ...
    "- After `createHtml`, use `read` on that file, then continue with `edit` or `patch` until the page is complete.",
    "",
