...
- The agent edits files only through the **workspace tools** in `src/agent/tools/` (`read`, `glob`, `grep`, `edit`, `write`, `previewRefresh`), registered by `createDesignPageWorkspaceTools` via `createProjectWorkspaceToolDefinitions` and `createWorkspaceToolRegistry`. These are sandboxed to the project workspace by `WorkspaceStore`, which resolves every tool path under the project root (`resolveProjectWorkspacePath` / `normalizeWorkspaceRelativePath` in `workspace-store/store.ts`). External font/icon/script CDNs are not hard-blocked by a guard module; instead the resource policy injected into the system prompt (derived from user settings in `settings-service.ts`) tells the agent which CDNs are approved.
