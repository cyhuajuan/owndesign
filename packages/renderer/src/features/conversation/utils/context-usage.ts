...
export function getUsedTokens(usage: ContextUsageMetadata | undefined) {
  ...
  return usage.totalTokens ?? addOptionalNumbers(usage.inputTokens, usage.outputTokens) ?? 0;
}

function addOptionalNumbers(first: number | undefined, second: number | undefined) {
