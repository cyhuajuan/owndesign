...
export function ModelSelect({
  ...
  onSelect: (modelId: string, thinkingMode?: DeepSeekThinkingMode) => void | Promise<void>;
            ...
            settings.modelConfigurations.map((configuration) =>
              configuration.provider === 'deepseek' ? (
                <DropdownMenuSub key={configuration.id}>
                  <DropdownMenuSubTrigger
                    className={modelMenuItemClass(configuration.id === selectedModelId)}
                    onClick={() => {
                      void onSelect(configuration.id);
                    }}
                  >
                    <ModelSelectCheck active={configuration.id === selectedModelId} />
                    <span className="min-w-0 flex-1 truncate">{configuration.model}</span>
                  </DropdownMenuSubTrigger>
                  <DropdownMenuSubContent
                    align="end"
                        ...
                        <DropdownMenuItem
                          className={modelMenuItemClass(
                            configuration.id === selectedModelId &&
                              getDeepSeekThinkingMode(configuration) === thinkingMode,
                          )}
                          key={thinkingMode}
                          onClick={() => {
...
function ModelSelectCheck({ active }: { active: boolean }) {
  return (
    <CheckIcon
      className={cn('size-3.5 shrink-0 opacity-0', active && 'text-[#6c5ce7] opacity-100')}
    />
  );
}
