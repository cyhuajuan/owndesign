...
export async function writeProjectWorkspaceFileWithCdnGuard(
  ...
  return workspaceStore.writeProjectWorkspaceFile(projectId, relativePath, guardedContent);
}
...
export async function editProjectWorkspaceFileWithCdnGuard(
  ...
  const content = await workspaceStore.readProjectWorkspaceFile(projectId, relativePath);
  const updatedContent = replaceInContent(content, oldString, newString, replaceAll, relativePath);
...
export async function applyProjectWorkspacePatchWithCdnGuard(
  ...
  return workspaceStore.applyProjectWorkspacePatch(projectId, changes, {
    transformContent: async (relativePath, content) =>
      guardIndexHtmlCdnChanges(workspaceStore, projectId, relativePath, content, approvedCdnUrls),
  });
}
...
async function guardIndexHtmlCdnChanges(
  ...
  const normalizedContent = normalizeConfiguredResourceCdnRefs(content, approvedCdnUrls);
  const approvedUrls = new Set(approvedCdnUrls.map(normalizeHttpsUrl).filter(Boolean));
...
function replaceInContent(
  ...
  if (content.indexOf(needle, firstIndex + needle.length) !== -1) {
    throw new Error(`oldString appears more than once in Project Workspace file: ${relativePath}`);
  }
...
function normalizeConfiguredResourceCdnRefs(html: string, approvedCdnUrls: string[]) {
